...
class JubaQLParser extends SqlParser with LazyLogging {
  ...
  protected lazy val createModel: Parser[JubaQLAST] = {
    ...
    CREATE ~> jubatusAlgorithm ~ MODEL ~ modelIdent ~ opt(labelOrId) ~ AS ~
      rep1sep(paramsAndFunction, ",") ~ CONFIG ~ stringLit ~ opt(RESOURCE ~ CONFIG ~> stringLit) ~
      opt(SERVER ~ CONFIG ~> stringLit) ~ opt(PROXY ~ CONFIG ~> stringLit) ^^ {
      case algorithm ~ _ ~ modelName ~ maybeLabelOrId ~ _ ~ l ~ _ ~ config ~ resourceConfig ~ serverConfig ~ proxyConfig =>
        CreateModel(algorithm, modelName, maybeLabelOrId, l, config, resourceConfig, serverConfig, proxyConfig)
    }
  }
