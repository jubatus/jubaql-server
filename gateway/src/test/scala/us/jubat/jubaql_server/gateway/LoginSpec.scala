...
class LoginSpec extends FlatSpec with Matchers with GatewayServer with BeforeAndAfter {
  ...
  "reconnect in persist" should "reboot gateway" in {
    ...
    val firstplan = new GatewayPlan("example.com", 1880,
      ...
      checkpointDir = "file:///tmp/spark", "localhost:8880", true, 16, 0, 0)
  ...
  "zookeeper connection failed" should "throw Exception" in {
    ...
    try {
      new GatewayPlan("example.com", 1237,
        ...
        sparkDistribution = "",
        fatjar = "src/test/resources/processor-logfile.jar",
        checkpointDir = "file:///tmp/spark", "localhost:9877", true, 16, 0, 0)
      fail()
    } catch {
      case e: Exception =>
  ...
  "POST to /login clustering" should "succeed" in {
    ...
    val node1_plan = new GatewayPlan("example.com", 1290,
      ...
      sparkDistribution = "",
      fatjar = "src/test/resources/processor-logfile.jar",
      checkpointDir = "file:///tmp/spark", clustername, true, 16, 0, 0)
  ...
  "spark-submit command succeed in productionMode" should "succeed" in {
    val url = :/("localhost", 1877) / "login"
    val (server, plan) = startServer(RunMode.Production("localhost:12181"), dummySparkPath)
    val req = Http(url.POST)
    req.either.apply() match {
      case Right(res) =>
        server.stop()
  ...
  "spark-submit command failed(fail sparkpath) in productionMode" should "fail" in {
    val url = :/("localhost", 1877) / "login"
    val (server, plan) = startServer(RunMode.Production("localhost:12181"), "failpath")
    val req = Http(url.POST)
    req.either.apply() match {
      case Right(res) =>
        server.stop()
  ...
  "spark-submit command failed(timeout) in productionMode" should "fail" in {
    ...
    val (server, plan) = startServer(RunMode.Production("localhost:12181"), dummySparkPath, "file:///timeout")
    val req = Http(url.POST)
    req.either.apply() match {
      case Right(res) =>
        server.stop()
    ...
    System.clearProperty("jubaql.gateway.submitTimeout")

  }

  "spark-submit command failed(return code: xx) in productionMode" should "fail" in {
    val url = :/("localhost", 1877) / "login"
    val (server, plan) = startServer(RunMode.Production("localhost:12181"), dummySparkPath, "file:///return")
    val req = Http(url.POST)
    req.either.apply() match {
      case Right(res) =>
        server.stop()
  ...
  "spark-submit command failed(after runnnig returun code:0) in productionMode" should "succeed" in {
    val url = :/("localhost", 1877) / "login"
    val (server, plan) = startServer(RunMode.Production("localhost:12181"), dummySparkPath, "file:///after")
    val req = Http(url.POST)
  ...
  "spark-submit command failed(after runnnig returun code:10) in productionMode" should "succeed" in {
    val url = :/("localhost", 1877) / "login"
    val (server, plan) = startServer(RunMode.Production("localhost:12181"), dummySparkPath, "file:///afterFailed")
    val req = Http(url.POST)
    val sessionId = req.either.apply() match {
      case Right(res) =>
        res.getStatusCode shouldBe(200)
  ...
  "spark-submit command failed(after runnnig throw Exception) in productionMode" should "succeed" in {
    val url = :/("localhost", 1877) / "login"
    val (server, plan) = startServer(RunMode.Production("localhost:12181"), dummySparkPath, "file:///exception")
    val req = Http(url.POST)
    val sessionId = req.either.apply() match {
      case Right(res) =>
        res.getStatusCode shouldBe(200)
  ...
  "resource for gateway(non DriverMemory/ExecutorMemory)" should "succeed" in {
    val url = :/("localhost", 1877) / "login"
    val plan = new GatewayPlan("localhost", 1877,
      Array(), RunMode.Production("localhost:12181", sparkDriverMemory=None, sparkExecutorMemory=None),
      sparkDistribution = dummySparkPath,
      fatjar = "src/test/resources/processor-logfile.jar",
      "file:///tmp/spark", "localhost:1877", false, 16, 0, 0)
    ...
    req.either.apply() match {
      case Right(res) =>
        ...
        plan.underlying.getMaxChannelMemorySize() shouldBe 0
        plan.underlying.getMaxTotalMemorySize() shouldBe 0
        plan.underlying.getCorePoolSize shouldBe 16

        // sparkDriverMemory, sparkExecutorMemory は目視確認
        // executing: --driver-memoryなし, --executor-memoryなし
  ...
  "resource for gateway(specified DriverMemory/ExecutorMemory)" should "succeed" in {
    val url = :/("localhost", 1877) / "login"
    val plan = new GatewayPlan("localhost", 1877,
      Array(), RunMode.Production("localhost:12181", sparkDriverMemory=Some("256M"), sparkExecutorMemory=Some("2G")),
      sparkDistribution = dummySparkPath,
      fatjar = "src/test/resources/processor-logfile.jar",
      "file:///tmp/spark", "localhost:1877", false, 32, 65536, 1048576)
    val server = unfiltered.netty.Server.http(1877).plan(plan)
    server.start()
    ...
    req.either.apply() match {
      case Right(res) =>
        ...
        res.getStatusCode shouldBe(200)
        res.getResponseBody should not be None
        println(s"${res.getResponseBody}")

        plan.underlying.getMaxChannelMemorySize() shouldBe 65536
        plan.underlying.getMaxTotalMemorySize() shouldBe 1048576
        plan.underlying.getCorePoolSize shouldBe 32

        // sparkDriverMemory, sparkExecutorMemory は目視確認
        // executing: --driver-memory 256M, --executor-memory 2G
    ...
    "spark-submit command succeed in productionMode without processor's logConf" should "succeed" in {
    //JubaQL Processor's log conf exist
    System.clearProperty("jubaql.processor.logconf")
//    System.setProperty("jubaql.processor.logconf", "file://path/processorLogConf_log4j.xml")
    val url = :/("localhost", 1877) / "login"
    val (server, plan) = startServer(RunMode.Production("localhost:12181"), dummySparkPath)
    val req = Http(url.POST)
    req.either.apply() match {
    ...
    // ログのexecuting: 行に以下が含まれて「いない」ことを確認
    // --files=file://path/processorLogConf_log4j.xml.*spark.driver.extraJavaOptions=.*-Dlog4j.configuration=processorLogConf_log4j.xml
  }

  "spark-submit command succeed in productionMode with processor's logConf" should "succeed" in {
    //JubaQL Processor's log conf exist
    System.clearProperty("jubaql.processor.logconf")
    System.setProperty("jubaql.processor.logconf", "file://path/processorLogConf_log4j.xml")
    val url = :/("localhost", 1877) / "login"
    ...
    // ログのexecuting: 行に以下が含まれて「いる」ことを確認
    // --files=file://path/processorLogConf_log4j.xml.*spark.driver.extraJavaOptions=.*-Dlog4j.configuration=processorLogConf_log4j.xml
  }

  "spark-submit command succeed in developmentMode without processor's logConf" should "succeed" in {
    //JubaQL Processor's log conf exist
    System.clearProperty("jubaql.processor.logconf")
//    System.setProperty("jubaql.processor.logconf", "file://path/processorLogConf_log4j.xml")
    val url = :/("localhost", 1877) / "login"
    ...
    server.stop()
    // ログのexecuting: 行に以下が含まれて「いる」ことを確認
    //--conf log4j.configuration=file:/tmp/log4j\d+.xml
  }

  "spark-submit command succeed in developmentMode with processor's logConf" should "succeed" in {
    //JubaQL Processor's log conf exist
    System.clearProperty("jubaql.processor.logconf")
    System.setProperty("jubaql.processor.logconf", "file://path/processorLogConf_log4j.xml")
    val url = :/("localhost", 1877) / "login"
    ...
    req.option.apply() should not be None
    server.stop()
    // ログでexecuting: 行に以下が含まれて「いる」ことを確認
    // --driver-java-options.*-Dlog4j.configuration=file://path/processorLogConf_log4j.xml
...
// ---------------------
  ...
  private def startServer(runMode:RunMode, sparkPath:String, checkpointDir: String = "file:///tmp/spark"): (Server, GatewayPlan) = {
    val plan = new GatewayPlan("localhost", 1877,
      ...
      fatjar = "src/test/resources/processor-logfile.jar",
      checkpointDir, "localhost:1877", false, 16, 0, 0)
    val server = unfiltered.netty.Server.http(1877).plan(plan)
    server.start()
    (server, plan)
  }
