...
class OrderedFileInputDStream[K: ClassTag, V: ClassTag, F <: NewInputFormat[K,V] : ClassTag](
    @transient ssc_ : StreamingContext,
    directory: String,
    filter: Path => Boolean = OrderedFileInputDStream.defaultFilter,
    newFilesOnly: Boolean = true)
  extends InputDStream[(K, V)](ssc_) {

  protected[streaming] override val checkpointData = new OrderedFileInputDStreamCheckpointData
  ...
  private def findNewFiles(currentTime: Long): (Seq[String], Long) = {
    ...
    // Heuristic: We look at files with a modification time less than five minutes
    // in the past. We therefore eliminate files that have seen updates in the
    // last five minutes, i.e. (hopefully) those files currently being appended to.
    val filter = new CustomPathFilter(currentTime - 5*60*1000)
    // sort the files from oldest to newest, just to be sure
    val fileStates = fs.listStatus(directoryPath, filter).sortBy(_.getModificationTime)
    val newFiles = fileStates.map(_.getPath.toString)
   ...
   */
  private[streaming]
  class OrderedFileInputDStreamCheckpointData extends DStreamCheckpointData(this) {
...
private[streaming]
object OrderedFileInputDStream {
