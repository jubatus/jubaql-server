...
class JubaQLParser extends SqlParser with LazyLogging {
  ...
  protected lazy val showQuery: Parser[JubaQLAST] = {
    SHOW ~> ident ^^ {
      case queryType =>
        ShowQuery(queryType.toLowerCase())
    }
  }

  protected lazy val showStreamsOrFunctionsQuery: Parser[JubaQLAST] = {
    SHOW ~> (SELECT | ANALYZE | WINDOW | TRIGGER | FEATURE) ~ ident ^^ {
      case queryType =>
        ShowQuery(s"${queryType._1} ${queryType._2}".toLowerCase())
    }
  }
