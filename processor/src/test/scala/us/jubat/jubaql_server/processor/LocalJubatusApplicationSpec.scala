...
import us.jubat.yarn.common.{LearningMachineType, ServerConfig, Mixer, LogConfig}
...
class LocalJubatusApplicationSpec extends FlatSpec with ShouldMatchers with BeforeAndAfterAll {
  ...
  "jubaanomaly" should "start" taggedAs (LocalTest, JubatusTest) in {
    val f = LocalJubatusApplication.start("foo", LearningMachineType.Anomaly, anomalyConfig, None)
  ...
  "jubaclassifier" should "start" taggedAs (LocalTest, JubatusTest) in {
    val f = LocalJubatusApplication.start("bar", LearningMachineType.Classifier, classifierConfig, None)
    Await.ready(f, Duration.Inf)
    val result = f.value.get
    result shouldBe a[Success[_]]
    result match {
  ...
  it should "be startable twice" taggedAs (LocalTest, JubatusTest) in {
    val f1 = LocalJubatusApplication.start("bar", LearningMachineType.Classifier, classifierConfig, None)
    Await.ready(f1, Duration.Inf)
    val result1 = f1.value.get
    val f2 = LocalJubatusApplication.start("bar", LearningMachineType.Classifier, classifierConfig, None)
  ...
  "jubarecommender" should "start" taggedAs (LocalTest, JubatusTest) in {
    val f = LocalJubatusApplication.start("baz", LearningMachineType.Recommender, recommenderConfig, None)
    Await.ready(f, Duration.Inf)
    val result = f.value.get
    result shouldBe a[Success[_]]
    result match {
  ...
  "start" should "return success for default server config" taggedAs (LocalTest, JubatusTest) in {
    // デフォルト
    val serverConfig = ServerConfig()
    val f = LocalJubatusApplication.start("test", LearningMachineType.Classifier, classifierConfig, None, serverConfig)
    Await.ready(f, Duration.Inf)
    val result = f.value.get
    result shouldBe a[Success[_]]
    result match {
    ...
    // コマンドのパラメータを目視確認
    // -c 2 -t 10 -x linear_mixer -s 16 -i 512 -Z 10 -I 10
  }

  it should "return success for customized server config" taggedAs (LocalTest, JubatusTest) in {
    // 値指定あり
    val serverConfig = ServerConfig(3, 30, Mixer.Broadcast, 0, 1024, 30, 40)
    val f = LocalJubatusApplication.start("test", LearningMachineType.Classifier, classifierConfig, None, serverConfig)
    Await.ready(f, Duration.Inf)
    val result = f.value.get
    result shouldBe a[Success[_]]
    ...
    // コマンドのパラメータを目視確認
    // -c 3 -t 30 -x broadcast_mixer -s 0 -i 1024 -Z 30 -I 40
  }

  it should "return success for default log config" taggedAs (LocalTest) in {
    val f = LocalJubatusApplication.start("test", LearningMachineType.Classifier, classifierConfig, None)
    Await.ready(f, Duration.Inf)
    val result = f.value.get
    result shouldBe a[Success[_]]
    result match {
    ...
    // コマンドのパラメータを目視確認
    // 以下が出力されていないことを確認
    // -g /tmp/jubatusServer_XXXXXX_log4j.xml
  }

  it should "return success log config jubatus_server: hdfs path" taggedAs (LocalTest) in {
    val logConfig = new LogConfig(None,None, Some("hdfs:///jubatus-on-yarn/test/js_log4j.xml"))
    val f = LocalJubatusApplication.start("test", LearningMachineType.Classifier, classifierConfig, Some(logConfig))
    Await.ready(f, Duration.Inf)
    val result = f.value.get
    result shouldBe a[Success[_]]
    result match {
  ...
  it should "return failed log config jubatus_server: not found path" taggedAs (LocalTest) in {
    val logConfig = new LogConfig(None,None, Some("hdfs:///jubatus-on-yarn/nonexist/js_log4j.xml"))
    val f = LocalJubatusApplication.start("test", LearningMachineType.Classifier, classifierConfig, Some(logConfig))
    Await.ready(f, Duration.Inf)
    val result = f.value.get
    result match {
      ...
      case Failure(t) =>
        t.printStackTrace()
        assert(t.isInstanceOf[java.io.FileNotFoundException])
    }
  }

  it should "return success log config jubatus_server: file path" taggedAs (LocalTest) in {
      val currentDir = new java.io.File(".").getCanonicalPath
    val logConfig = new LogConfig(None,None, Some(s"file://${currentDir}/src/test/resources/js_log4j.xml"))
    val f = LocalJubatusApplication.start("test", LearningMachineType.Classifier, classifierConfig, Some(logConfig))
    Await.ready(f, Duration.Inf)
    val result = f.value.get
    result shouldBe a[Success[_]]
  ...
  it should "return success log config jubatus_server: no scheme" taggedAs (LocalTest) in {
    //スキームなしはdefaultFSが適用される
    val logConfig = new LogConfig(None,None, Some("/jubatus-on-yarn/test/js_log4j.xml"))
    val f = LocalJubatusApplication.start("test", LearningMachineType.Classifier, classifierConfig, Some(logConfig))
    Await.ready(f, Duration.Inf)
    val result = f.value.get
    result shouldBe a[Success[_]]
  ...
  it should "return failed log config jubatus_server: illegal scheme" taggedAs (LocalTest) in {
    val logConfig = new LogConfig(None,None, Some("hoge:///jubatus-on-yarn/test/js_log4j.xml"))
    val f = LocalJubatusApplication.start("test", LearningMachineType.Classifier, classifierConfig, Some(logConfig))
    Await.ready(f, Duration.Inf)
    val result = f.value.get
    result match {
      ...
      case Failure(t) =>
        t.printStackTrace()
        assert(t.isInstanceOf[java.io.IOException])
    }
  }
