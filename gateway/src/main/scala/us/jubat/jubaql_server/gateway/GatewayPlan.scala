...
object RunMode {
  case class Production(zookeeper: String, numExecutors: Int = 3, coresPerExecutor: Int = 2,
                        ...
                        sparkExecutorMemory: Option[String] = None) extends RunMode("Cluster") {
    override def toString(): String = s"$name($zookeeper,$numExecutors,$coresPerExecutor,$sparkJar,$sparkDriverMemory,$sparkExecutorMemory)"
  }
  case class Development(numThreads: Int = 3) extends RunMode("Local") {
    override def toString(): String = s"$name($numThreads)"
  }
